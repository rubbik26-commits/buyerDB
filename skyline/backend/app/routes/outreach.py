"""Outreach API over live sbi_* tables.

Builds broker-reviewable target lists and persists drafts from contacts + real
buyer history. No email is sent and no contact information is invented.
"""
        ...
        cur.execute(f"""
            WITH buyer_activity AS (
              SELECT e.entity_id, e.display_name AS name, e.is_spv_suspect,
                     count(DISTINCT d.deal_id) AS deal_count, coalesce(sum(d.sale_price),0) AS volume,
              ...
              FROM sbi_contacts GROUP BY entity_id
            )
...
def drafts(user_id: str = "broker", limit: int = Query(50, ge=1, le=200)):
    with db() as conn, conn.cursor() as cur:
        cur.execute("""
            ...
            FROM sbi_outreach_drafts od JOIN sbi_entities e USING (entity_id)
            WHERE od.user_id=%s
...
def draft(body: DraftRequest):
    with db() as conn, conn.cursor() as cur:
        cur.execute("SELECT entity_id, display_name FROM sbi_entities WHERE entity_id=%s", (body.entity_id,))
        ent = rows(cur)
        if not ent:
        ...
        cur.execute("""
            ...
            FROM sbi_contacts WHERE entity_id=%s
