...
Builds broker-reviewable target lists and persists drafts from contacts + real buyer history.
No email is sent and no contact information is invented.
"""
...
@router.get("/targets")
def targets(asset_type: Optional[str] = None, borough: Optional[str] = None, price_min: Optional[float] = None, price_max: Optional[float] = None, require_email: bool = False, limit: int = Query(75, ge=1, le=250)):
    where = ["dp.role='buyer'"]
    params = []
    if asset_type:
        where.append("d.asset_type=%s"); params.append(asset_type)
    if borough:
        where.append("p.borough=%s"); params.append(borough)
    if price_min is not None:
        where.append("d.sale_price >= %s"); params.append(price_min)
    if price_max is not None:
        where.append("d.sale_price <= %s"); params.append(price_max)
        ...
        cur.execute(f"""
            WITH buyer_activity AS (
              SELECT e.entity_id, e.display_name AS name, e.is_spv_suspect,
                     count(d.deal_id) AS deal_count, coalesce(sum(d.sale_price),0) AS volume,
              ...
              FROM deal_parties dp JOIN entities e USING (entity_id) JOIN deals d USING (deal_id) JOIN properties p USING (property_id)
              WHERE {' AND '.join(where)}
              GROUP BY e.entity_id, e.display_name, e.is_spv_suspect
            ), contact_counts AS (
              SELECT entity_id, count(*) AS contact_count,
                     count(*) FILTER (WHERE nullif(email,'') IS NOT NULL) AS email_count,
                     count(*) FILTER (WHERE nullif(phone,'') IS NOT NULL) AS phone_count
              FROM contacts GROUP BY entity_id
            )
            SELECT ba.*, coalesce(cc.contact_count,0) AS contact_count, coalesce(cc.email_count,0) AS email_count, coalesce(cc.phone_count,0) AS phone_count
            FROM buyer_activity ba LEFT JOIN contact_counts cc USING (entity_id)
            {email_filter}
...
def drafts(user_id: str = "broker", limit: int = Query(50, ge=1, le=200)):
    with db() as conn, conn.cursor() as cur:
        cur.execute("""
            ...
            FROM outreach_drafts od JOIN entities e USING (entity_id)
            WHERE od.user_id=%s
