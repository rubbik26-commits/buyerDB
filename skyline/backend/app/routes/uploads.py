"""uploads.py — user CSV/Excel contact ingestion into the live sbi_* schema."""
import io, json
import pandas as pd
    ...
    for col in columns:
        ...
        for canon, syns in SYNONYMS.items():
            ...
            if c == canon or c in syns:
                ...
                for s in syns:
                    s_norm = s.replace("-", "_")
                    if "_" in s_norm and s_norm in c:
                        score = max(score, 2)
                    elif s_norm in tokens:
                        score = max(score, 1)
            if score > best_score:
...
async def create_upload(file: UploadFile = File(...), user_id: str = Form("system")):
    ...
    with db() as conn, conn.cursor() as cur:
        cur.execute("""INSERT INTO sbi_uploads (user_id, filename, row_count, column_mapping, status)
                       VALUES (%s,%s,%s,%s::jsonb,'staged') RETURNING upload_id""",
                    (user_id, file.filename, len(df), json.dumps(mapping)))
        upload_id = cur.fetchone()[0]
        for i, row in df.iterrows():
            cur.execute("INSERT INTO sbi_upload_rows (upload_id, row_num, raw) VALUES (%s,%s,%s::jsonb)",
...
def resolve_upload(body: MappingConfirm):
    ...
    with db() as conn, conn.cursor() as cur:
        cur.execute("SELECT status FROM sbi_uploads WHERE upload_id=%s", (body.upload_id,))
        u = cur.fetchone()
        if not u:
            return {"error": "upload not found"}
        if u[0] == "imported":
            return {"error": "upload already imported", "upload_id": body.upload_id}
        cur.execute("UPDATE sbi_uploads SET column_mapping=%s::jsonb, status='resolving' WHERE upload_id=%s",
                    (json.dumps(body.mapping), body.upload_id))
        cur.execute("""SELECT row_num, raw FROM sbi_upload_rows
        ...
        for row_num, raw in staged:
            ...
            if not ename or is_placeholder(str(ename)):
                ...
                cur.execute("UPDATE sbi_upload_rows SET status='rejected', resolution=%s::jsonb WHERE upload_id=%s AND row_num=%s",
            ...
            if res["status"] == "needs_review":
                ...
                cur.execute("UPDATE sbi_upload_rows SET status='needs_review', resolution=%s::jsonb WHERE upload_id=%s AND row_num=%s",
                            (json.dumps(res), body.upload_id, row_num))
                cur.execute("""INSERT INTO sbi_review_queue (object_type, object_id, issue_class, payload)
                               VALUES ('upload_row', %s, 'entity_merge', %s::jsonb)""",
            ...
            if res["status"] == "auto_matched":
                ...
                cur.execute("""INSERT INTO sbi_entities (display_name, entity_type)
                               VALUES (%s,%s) ON CONFLICT (norm_name) DO UPDATE SET updated_at=now()
                               RETURNING entity_id""", (str(ename).strip(), entity_type(nn)))
        ...
        cur.execute("UPDATE sbi_uploads SET status='imported' WHERE upload_id=%s", (body.upload_id,))
...
def _import_contact(cur, entity_id, raw, val, body, stats):
    phone = val(raw, "phone")
    email = val(raw, "email")
    person = val(raw, "person_name")
    title = val(raw, "title")
    mail = val(raw, "mailing_address")
    if any([phone, email, person, mail]):
        cur.execute("""INSERT INTO sbi_contacts (entity_id, person_name, title, phone, email,
                                             mailing_address, source, provenance)
                       VALUES (%s,%s,%s,%s,%s,%s,%s,%s::jsonb)""",
                    (entity_id, person, title, phone, email, mail,
                     f"upload:{body.upload_id}", json.dumps({"created_by": body.user_id})))
        stats["contacts_created"] += 1
    when = val(raw, "last_contact_date")
    notes = val(raw, "interaction_notes")
    ...
    if when or notes:
        ...
        if ts:
            ...
            cur.execute("""INSERT INTO sbi_interactions (entity_id, user_id, channel, occurred_at, notes)
...
def list_uploads():
    with db() as conn, conn.cursor() as cur:
        cur.execute("""SELECT upload_id, filename, row_count, status, created_at
                       FROM sbi_uploads ORDER BY created_at DESC LIMIT 50""")
