...
export async function runRolling(options: Json = {}) {
  ...
  const targets = await runtimeRpc("sbi_runtime_rolling_targets", { p_lim: limit });
  ...
  for (const target of targets) {
    ...
    if (!number || !street || !token || !BOROUGH_CODE[target.borough]) {
      stats.nohit++;
      await runtimeRpc("sbi_runtime_rolling_apply", { p_deal_id: target.deal_id, p_sqft: null, p_units: null, p_source_rows: [] });
      continue;
    }
    try {
      ...
      const result = await runtimeRpc("sbi_runtime_rolling_apply", { p_deal_id: target.deal_id, p_sqft: sqft, p_units: units, p_source_rows: matches });
