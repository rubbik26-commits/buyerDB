...
export function EntityDrawer({ entityId, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(18,22,31,0.45)", zIndex: 50, display: "flex", justifyContent: "flex-end" }} onClick={onClose}>
      <div style={{ width: "min(560px, 94vw)", background: "var(--paper)", height: "100%", overflowY: "auto", boxShadow: "var(--shadow-lg)", padding: "24px 26px" }} onClick={(e) => e.stopPropagation()}>
        ...
        {data && data.entity && <>
          <div className="view-head" style={{ marginBottom: 14 }}><div className="eyebrow">{data.entity.entity_type || "entity"}{data.entity.is_spv_suspect ? " · single-purpose LLC" : ""}</div><h1 style={{ fontSize: 24 }}>{data.entity.display_name}</h1></div>
          ...
          {contacts.length === 0 ? <div className="banner warn" style={{ marginBottom: 16 }}>No phone or email on file. Public records cap at mailing addresses; phone and email exist only where a source published them or a broker uploaded them.</div> : contacts.map((c, i) => <div className="cite" key={i} style={{ marginBottom: 6 }}><span className="nm">{c.person_name || "-"}</span><span className="mono">{c.phone || "no phone"} · {c.email || "no email"} · {c.source || "source unknown"}</span></div>)}
