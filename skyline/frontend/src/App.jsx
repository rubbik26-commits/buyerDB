  ...
  return (
    <div className="app">
      <aside className="rail">
        ...
        <div className="foot"><div className="k">DATASET</div><div className="v">{s ? num(s.deals) + " deals" : "..."}</div><div className="k" style={{ marginTop: 8 }}>COVERAGE</div><div className="v">{s && s.earliest ? `${String(s.earliest).slice(0,4)}-${String(s.latest).slice(0,4)}` : "-"}</div></div>
      </aside>
      <main className="main">
        <div className="pulse">
...
function Cell({ lab, val, accent }) { return <div className="cell"><div className="lab">{lab}</div><div className="val">{accent ? <em>{val}</em> : val}</div></div>; }
