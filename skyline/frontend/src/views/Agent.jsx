...
export default function Agent() {
  ...
  async function ask(q) {
    ...
    try {
      ...
      if (res.error) {
        setMsgs((m) => [...m, { role: "agent", error: res.detail || res.error, hint: res.hint }]);
      } else {
        setMsgs((m) => [...m, { role: "agent", tool: res.tool, args: res.arguments, why: res.plan_why, result: res.result, answer: res.answer, providers: res.providers }]);
      }
    } catch (e) {
        ...
        <div className="agent-inner">
          {msgs.length === 0 && (
            <div style={{ paddingTop: 20 }}>
              ...
              <p style={{ color: "var(--tx-dim)", fontSize: 14, maxWidth: "60ch" }}>
                Ask in plain language. The desk translates your question into a database query,
                runs it, and answers from the rows — citing them. Contact details come only from
                records on file; it never invents a phone number.
              </p>
              <div className="suggests">
                {SUGGESTS.map((s) => <span key={s} className="suggest" onClick={() => ask(s)}>{s}</span>)}
              </div>
            </div>
          )}
              ...
              {m.role === "user" ? (
                ...
                <div className="bubble">
                  <div className="banner err" style={{ margin: 0 }}>
                    The desk needs an AI provider key to answer. {m.hint || ""}
                    <div style={{ marginTop: 6, fontFamily: "var(--mono)", fontSize: 11 }}>{m.error}</div>
                  </div>
                </div>
              ) : (
                <div className="bubble">
                  {m.tool && (
                    <div className="trace">
                      <div className="th">Query plan{m.providers?.plan ? ` · ${m.providers.plan}` : ""}</div>
                      <div className="tool">{m.tool}()</div>
                      {m.args && Object.keys(m.args).length > 0 && (
                        <div className="args">{JSON.stringify(m.args)}</div>
                      )}
                    </div>
                  )}
      ...
      <div className="agent-input">
        <div className="row">
          <textarea
            ...
            placeholder="Ask about buyers, owners, contacts, or recent deals…"
            onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); ask(); } }}
          />
          <button className="btn brass" onClick={() => ask()} disabled={busy || !input.trim()}>Ask</button>
        </div>
      </div>
    </div>
  );
...
/* Render the raw tool rows as citations the broker can verify — the answer's evidence. */
